...
void Window::ManejaTeclado(GLFWwindow* window, int key, int code, int action, int mode)
{
	...
	if (key == GLFW_KEY_Y)
	{
		...
		theWindow->luzfaroD = false;
		theWindow->luzfaroT = true;
	}
	if (key == GLFW_KEY_U)
	{
		theWindow->muevex -= 1.0;
		theWindow->luzfaroD = true;
		theWindow->luzfaroT = false;
	}
	...
	{
		theWindow->luzfaroA = !theWindow->luzfaroA;
	}

	if (key == GLFW_KEY_H)
	{
		if (theWindow->articulacion3 < 55.0f)
