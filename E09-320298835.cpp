...
int main()
{
	...
	Dragon_M = Model();
	Dragon_M.LoadModel("Models/Tiamatsinalas.obj");
	Alas = Model();
	Alas.LoadModel("Models/Ala.obj");
	AlaD = Model();
	...
	while (!mainWindow.getShouldClose())
	{
		...
		movOffsetDragon = 0.8f;
		// dragon avance
		if (avanzadragon)
		{
			if (movDragon < 20.0f)
			{
				movDragon += movOffsetDragon * deltaTime2;
			}
			else
			{
				avanzadragon = false;
		...
		// aleteo
		model = glm::rotate(model, glm::radians(sin(angulovaria * 1.0f) * 30.0f), glm::vec3(1.0f, 0.0f, 0.0f));
		model = glm::translate(model, glm::vec3(2.0f, 0.0f, 0.0f));
		Material_brillante.UseMaterial(uniformSpecularIntensity, uniformShininess);
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		Alas.RenderModel();
